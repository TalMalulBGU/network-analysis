...
TEMPLATE_BATCH_FILE="/home/talmalu/thesis/projects/python/NetworkAnalysis/Scripts/template_batches/density_between_taxonomies_heatmap_batch_template.sbatch"
...
while [[ $# -gt 0 ]]; do
	...
	-c|--edges_count_xlsx)
		...
		shift
    ;;
	-p|--edges_possible_count_xlsx)
		POSSIBLE_COUNT_XLSX=$2
		shift
		shift
    ;;
	-l|--taxonomy_level)
...
sed -i "s~###ACTUAL_COUNT_XLSX###~$ACTUAL_COUNT_XLSX~g" $TMP_FILE_PATH
sed -i "s~###POSSIBLE_COUNT_XLSX###~$POSSIBLE_COUNT_XLSX~g" $TMP_FILE_PATH
