...
def _main():
	...
	parser.add_argument('--edges_count_left_xlsx', help='number of taxonomy to calculate', type=str, required=True)
	parser.add_argument('--edges_possible_count_left_xlsx', help='number of taxonomy to calculate', type=str, required=True)
	parser.add_argument('--edges_count_right_xlsx', help='number of taxonomy to calculate', type=str, required=True)
	parser.add_argument('--edges_possible_count_right_xlsx', help='number of taxonomy to calculate', type=str, required=True)
	...
	print("edges_count_left_xlsx = {}".format(options.edges_count_left_xlsx))
	print("edges_possible_count_left_xlsx = {}".format(options.edges_possible_count_left_xlsx))
	print("edges_count_right_xlsx = {}".format(options.edges_count_right_xlsx))
	print("edges_possible_count_right_xlsx = {}".format(options.edges_possible_count_right_xlsx))
	...
	taxonomy_links_left = pd.read_excel(options.edges_count_left_xlsx, index_col=0).astype(float)
	possible_taxonomy_links_left = pd.read_excel(options.edges_possible_count_left_xlsx, index_col=0).astype(float)
	taxonomy_links_right = pd.read_excel(options.edges_count_right_xlsx, index_col=0).astype(float)
	possible_taxonomy_links_right = pd.read_excel(options.edges_possible_count_right_xlsx, index_col=0).astype(float)	
	
	df = (taxonomy_links_left / possible_taxonomy_links_left) - (taxonomy_links_right / possible_taxonomy_links_right)
