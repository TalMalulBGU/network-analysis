...
def _main():
	...
	for idx_source in range(options.start, stoping_index, 1):
		...
		for idx_target in range(idx_source + 1, len(database.dataframe.index)):
			...
			alignment = align.globalds(source_sequence, target_sequence, matrix, options.gap_open_penalty, options.gap_extend_penalty, one_alignment_only=True)[0]
			...
			alignment_scores.append({'source': source_name, 'target': target_name, '{}_score'.format(options.alignment_name): alignment.score, '%{}_id'.format(options.alignment_name): alignment_identity , 'normalized_{}_score'.format(options.alignment_name): normalized_alignment_score, 'alignment_length': alignment_length})
