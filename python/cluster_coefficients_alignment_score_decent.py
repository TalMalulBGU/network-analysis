...
def _main():
	...
	while alignment_score <= max_alignment_score:
		ig_graph = ig_graph.subgraph_edges(ig_graph.es.select(lambda e: e[options.weight] >= alignment_score))
		clustering_coefficient = ig_graph.transitivity_undirected()
		clustering_coefficients.append(clustering_coefficient)
		n_vertices.append(ig_graph.vcount())
		n_edges.append(ig_graph.ecount())
		alignment_score+=1


	result_dictionary = {}
	for i in range(min_alignment_score, max_alignment_score + 1):
		result_dictionary[i] = {'coefficient': clustering_coefficients[i - min_alignment_score],
								'n_vertices': n_vertices[i - min_alignment_score],
								'n_edges': n_edges[i - min_alignment_score]
							   }
