...
def _main():
	...
	title = 'Density - Degree Descent'
	degrees = []
	densities = []

	with open(options.data_file) as json_file:
		data = json.load(json_file)
		degrees = list(map(int,data.keys()))
		densities = data.values()

	ax.plot(degrees, densities, '.-')
	ax.set_title(title)
	...
	plt.savefig(options.out_file, bbox_inches='tight', transparent=True, dpi=300)
	plt.show()
