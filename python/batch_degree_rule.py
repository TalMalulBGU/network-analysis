...
def _main():
	...
	parser.add_argument('--degrees', help='degree file location', type=int, nargs='+', required=True)
	...
	print("degrees = {}".format(options.degrees))
	...
	for degree in options.degrees:
		result_dictionary[degree] = {}
		result_dictionary[degree]['function_clustering'] = {}
		result_dictionary[degree]['taxonomy_clustering'] = {}
		
		graph_filter = FilterFactory.create_filter('degree', degree_rule = options.rule, degree = degree, delete_vertices = options.delete_vertices)
		...
		for taxonomy_level, analysis_result in taxonomy_clustering_result.items():
			result_dictionary[degree]['taxonomy_clustering'][taxonomy_level] = analysis_result.to_dict()
		...
		for analysis in options.analyses:
			...
			result_dictionary[degree]['function_clustering'][algoritm_name] = algoritm_analysis.to_dict()
			for function_relevancy_str in options.relevances:
				...
				result_dictionary[degree]['function_clustering'][algoritm_name][relevance_name] = relevancy_analysis.to_dict()
				result_dictionary[degree]['function_clustering'][algoritm_name][relevance_name]['relevance_xlsx'] = relevance_xlsx
				result_dictionary[degree]['function_clustering'][algoritm_name][relevance_name]['reviewed'] = reviewed.lower() == 'reviewed'
		...
		result_dictionary[degree]['database'] = options.database
		result_dictionary[degree]['xgmml_file'] = options.xgmml_file
		result_dictionary[degree]['graph_analysis'] = simple_result.to_dict()		
