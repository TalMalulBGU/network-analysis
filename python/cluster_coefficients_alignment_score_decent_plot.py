...
def _main():
	...
	with open(options.data_file) as json_file:
		...
		for value in data.values():
			clustering_coefficients.append(value['coefficient'])
			n_vertices.append(value['n_vertices'])
			n_edges.append(value['n_edges'])
		

		
	fig, ax_coefficient = plt.subplots(1, 1, figsize=(20,6))
	ax_vertices = ax_coefficient.twinx()
	ax_edges = ax_coefficient.twinx()

	ax_edges.spines.right.set_position(("axes", 1.1))


	ln_coefficient = ax_coefficient.plot(alignment_scores, clustering_coefficients, marker='.', label='Clustering Coefficient')
	ln_vertices = ax_vertices.plot(alignment_scores, n_vertices, marker='.', color='r', label='Vertices')
	ln_edges = ax_edges.plot(alignment_scores, n_edges, marker='.', color='g', label='Edges')


	ax_coefficient.set_title('Clustering Coefficient over different Alignment Score threshold')
	ax_coefficient.set_xlabel('Alignment Score')
	ax_coefficient.set_ylabel('Clustering Coefficient')

	ax_vertices.set_ylabel('# Vertices')
	ax_edges.set_ylabel('# Edges')

	lns = ln_coefficient +  ln_vertices + ln_edges
	labs = [l.get_label() for l in lns]
	ax_coefficient.legend(lns, labs)
