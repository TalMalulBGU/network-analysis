...
def only_in_or_filter_network_by_attributes(graph, filter_dict):
	vs_filtered = set()
...
def any_in_or_filter_network_by_attributes(graph, filter_dict):
	vs_filtered = set()
	for attribute, values in filter_dict.items():
		for value in values:
			vs_filtered.update(graph.vs.select(lambda v: any(a == value for a in v[attribute])))
	graph = graph.subgraph(vs_filtered)
	return graph
...
def _main():
	...
	parser.add_argument('--n_top', help='number of taxonomy to calculate', type=int, default=4)
	...
	print("n_top = {}".format(options.n_top))
	...
	taxonomy_counter = Counter(e for l in graph.vs[options.taxonomy_level] for e in l)
	taxonomy_colors = dict(zip(df_relevancy.index, df_relevancy['Color']))
	values = list(dict(taxonomy_counter.most_common()).values())
	top_keys = list(dict(taxonomy_counter.most_common()[:options.n_top]).keys())
	none_color = taxonomy_colors.get('None', 'black')
	taxonomy_bar_colors = [taxonomy_colors.get(ep, none_color) for ep in top_keys]

	total_values_sum = sum(values)
	top_values = values[:options.n_top]
	other_value = (total_values_sum - sum(top_values)) / total_values_sum
	taxonomy_bar_colors.append('whitesmoke')

	top_values = list(np.array(values[:options.n_top]) / total_values_sum)
	top_values.append(other_value)
	top_keys.append('Other')
	
	
	fig, ax = plt.subplots(figsize=(10,10))

	ax.pie(top_values , labels=top_keys, colors=taxonomy_bar_colors, textprops={'fontsize': 14}, autopct='%1.1f%%')
	ax.set_title('Distribution of {}'.format(options.taxonomy_level))
