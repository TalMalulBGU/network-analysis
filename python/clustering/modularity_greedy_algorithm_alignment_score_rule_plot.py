...
def _main():
	...
	with open(options.data_file) as json_file:
		...
		for value in data.values():
			modularities.append(value['modularity'])
			n_edges.append(value['n_edges'])
			n_vertices.append(value['n_vertices'])
			n_clusters.append(value['n_clusters'])
	...
	ax.plot(alignment_scores, modularities, '.-', label='{}'.format('Greedy Modularity'))
	for i in range(0, len(n_clusters), 10):
		ax.annotate(str(n_clusters[i]), (alignment_scores[i], modularities[i]))


	ax_edges.plot(list(map(int, n_alignment_scores_edges.keys())), list(map(int, n_alignment_scores_edges.values())), '.-', color='black', label='edges({:,})'.format(n_max_edges))
