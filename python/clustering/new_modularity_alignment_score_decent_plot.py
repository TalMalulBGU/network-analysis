...
def _main():
	...
	parser.add_argument('--analysis_files', help='name of edge attribute', type=str, nargs='+', required=True)
    ...
    for analysis_file in options.analysis_files:
        with open(analysis_file) as json_file:
            data = json.load(json_file)
            alignment_scores = data.keys()
            for alignment_score in alignment_scores:
                alignment_score_data = data[str(alignment_score)]
                taxonomy_level_modularity = {}
                algorithm_modularity = {}
                
                for taxonomy_level in alignment_score_data['taxonomy_clustering'].keys():
                    taxonomy_level_size[taxonomy_level] = int(alignment_score_data['taxonomy_clustering'][taxonomy_level]['n_clusters'])
                    taxonomy_level_modularity[taxonomy_level] = alignment_score_data['taxonomy_clustering'][taxonomy_level]['scores']['modularity']
                    
                taxonomy_modularities.append(taxonomy_level_modularity)
                
                
                for algorithm_name in alignment_score_data['function_clustering'].keys():
                    taxonomy_level_size = alignment_score_data['function_clustering'][algorithm_name]['n_clusters']
                    algorithm_modularity[algorithm_name] = {
                        'modularity': alignment_score_data['function_clustering'][algorithm_name]['scores']['modularity'],
                        'n_clusters': alignment_score_data['function_clustering'][algorithm_name]['n_clusters']
                    }
                    
                algorithm_modularities.append(taxonomy_level_modularity)


            
    taxonomy_modularity = pd.DataFrame.from_dict({k: v for k, v in zip(alignment_scores, taxonomy_modularities)}, orient='index')
	...
	for taxonomy_level in taxonomy_modularity.columns:
		n_clusters = taxonomy_level_size[taxonomy_level]
