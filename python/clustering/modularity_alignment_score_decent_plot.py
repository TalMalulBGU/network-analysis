...
def _main():
	...
	parser.add_argument('--taxonomy_data_file', help='name of edge attribute', type=str, required=True)
	parser.add_argument('--greedy_data_file', help='name of edge attribute', type=str, required=False)
	parser.add_argument('--leiden_data_file', help='name of edge attribute', type=str, required=False)
	...
	print("taxonomy_data_file = {}".format(options.taxonomy_data_file))
	print("greedy_data_file = {}".format(options.greedy_data_file))
	print("leiden_data_file = {}".format(options.leiden_data_file))
	...
	taxonomy_alignment_scores = []
	greedy_alignment_scores = []
	taxonomy_modularities = []
	greedy_modularities = []
	leiden_modularities = []
	n_edges = []
	greedy_n_vertices = []
	...
	with open(options.taxonomy_data_file) as json_file:
		data = json.load(json_file)
		taxonomy_alignment_scores = list(map(int,data.keys()))
		for result_dict in list(map(dict,data.values())):
			taxonomy_modularities.append(result_dict['modularity'])
			n_edges.append(result_dict['n_edges'])
		
		n_max_edges = max(n_edges)
		taxonomy_modularity = pd.DataFrame.from_dict({k: v for k, v in zip(taxonomy_alignment_scores, taxonomy_modularities)}, orient='index')
		n_alignment_scores_edges = {k: v for k, v in zip(taxonomy_alignment_scores, n_edges)}
		
		
	with open(options.greedy_data_file) as json_file:
		data = json.load(json_file)
		greedy_alignment_scores = sorted(list(map(int,data.keys())))
		for alignment_score in greedy_alignment_scores:
			greedy_modularities.append(data[str(alignment_score)]['modularity'])
			greedy_n_vertices.append(data[str(alignment_score)]['n_vertices'])
			greedy_n_clusters.append(data[str(alignment_score)]['n_clusters'])
			
	with open(options.leiden_data_file) as json_file:
		data = json.load(json_file)
		leiden_alignment_scores = sorted(list(map(int,data.keys())))
		for alignment_score in leiden_alignment_scores:
			leiden_modularities.append(data[str(alignment_score)]['quality'])
			leiden_n_vertices.append(data[str(alignment_score)]['n_vertices'])
			leiden_n_clusters.append(data[str(alignment_score)]['n_clusters'])
	   
	
	ax_edges = ax.twinx()
	for taxonomy_level in taxonomy_modularity.columns:
		n_clusters = len(Counter([taxonomy for lst_taxonomy in graph.vs[taxonomy_level] for taxonomy in lst_taxonomy]))
		ax.plot(list(taxonomy_modularity.index), taxonomy_modularity[taxonomy_level], '.-', label='{}({})'.format(taxonomy_level, n_clusters))
		
	ax.plot(greedy_alignment_scores, greedy_modularities, '.-', label='{}'.format('Greedy Modularity'))
	for i in range(0, len(greedy_n_clusters), 10):
		ax.annotate(str(greedy_n_clusters[i]), (greedy_alignment_scores[i], greedy_modularities[i]))
		
	ax.plot(leiden_alignment_scores, leiden_modularities, '.-', label='{}'.format('Leiden Modularity'))
	for i in range(0, len(leiden_n_clusters), 10):
		ax.annotate(str(leiden_n_clusters[i]), (leiden_alignment_scores[i], leiden_modularities[i]))


	ax_edges.plot(taxonomy_modularity.index, list(map(int, n_alignment_scores_edges.values())), '.-', color='black', label='edges({:,})'.format(n_max_edges))
