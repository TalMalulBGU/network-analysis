...
def community_function_plot(ax, clusters, db, graph, title, modularity_score, taxonomy_level, function_relevancy):

	cluster_function_dictionary = {}
	f_col_name = 'Gene ontology (molecular function)'
	for cluster_id, cluster in clusters.items():
		cluster_vertices = graph.vs[cluster]
		if len(cluster_vertices) > 0:
			entry_db = db.loc[cluster_vertices['name']]
			...
			function_counter = Counter(f for l in entry_relevant_function_db[f_col_name] for f in l if f in list(function_relevancy.index))
			cluster_function_dictionary[cluster_id] = {'Counter': function_counter, 'Total_Nodes': len(cluster_vertices), 'Function_Nodes': len(entry_relevant_function_db)}
	print(cluster_function_dictionary)


	taxonomies = list(dict([taxonomy for cluster in cluster_function_dictionary.values() for taxonomy in cluster['Counter'].most_common()]).keys())
	clusters_taxonomies = []
	buttom = list(np.zeros(len(cluster_function_dictionary.keys())))
	d_taxonomy_colors = dict(zip(function_relevancy.index, function_relevancy.loc[function_relevancy.index, 'Color']))
	d_taxonomy_colors = dict(sorted(d_taxonomy_colors.items(), key=lambda item: item[1]))

	for idx, taxonomy in enumerate(taxonomies):
		clusters_taxomomy = []
		for cluster_id in cluster_function_dictionary.keys():
			clusters_taxomomy.append(cluster_function_dictionary[cluster_id]['Counter'].get(taxonomy, 0))
		clusters_taxonomies.append(clusters_taxomomy)

		ax.bar(list(map(str, cluster_function_dictionary.keys())), clusters_taxomomy, bottom=buttom, color=d_taxonomy_colors.get(taxonomy,'black'), label=taxonomy)
		buttom = [buttom[i] + v for i, v in enumerate(clusters_taxomomy)]


	ax.set_xticklabels(['{}\n{}/{}'.format(cid, d['Function_Nodes'], d['Total_Nodes']) for cid, d in cluster_function_dictionary.items()])
...
def _main():
	...
	parser.add_argument('--function_column', help='the function column for db and relevance', type=str, required=True)
	parser.add_argument('--relevance_function_xlsx', help='the taxonomy relevancy group', required=True, type=str)
	...
	print("function_column = {}".format(options.function_column))
	...
	df_relevancy = read_relevancy(options.relevance_function_xlsx)
	df_relevancy = df_relevancy[df_relevancy['relevance']]
	df_relevancy = df_relevancy.set_index(options.function_column)
	fig, ax = plt.subplots(1, 1, figsize=(int(40), 8))
	community_function_plot(ax, clusters, uniprot_db, graph, options.image_title, algorithm.modularity, options.function_column, df_relevancy)
	plt.show()		
