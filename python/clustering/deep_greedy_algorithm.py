...
def _main():
	...
	parser.add_argument('--algorithm_file', help='name of edge attribute', type=str, required=False)
	...
	print("graph_name = " + str(options.graph_name))
	...
	if options.algorithm_file:
		...
		algorithm = cluster_community_dendogram.as_clustering()
		write_cluster(graph, algorithm, '{}/{}.json'.format(options.output_folder, options.graph_name))
	
	clustering_dictionary_queue.append((algorithm, options.graph_name))
	
	while(len(clustering_dictionary_queue) > 0):
		algorithm, parent = clustering_dictionary_queue.popleft()
		...
		for cluster_id, cluster in clusters.items():
			...
			cluster_community_dendogram = cluster_graph.community_fastgreedy(weights=options.alignment_weight)
			cluster_community_vc = cluster_community_dendogram.as_clustering()
			clustering_dictionary_queue.append((cluster_community_vc, '{}_{}'.format(parent, cluster_id)))
			
			write_cluster(cluster_graph, cluster_community_vc, '{}/{}_{}.json'.format(options.output_folder, parent, cluster_id))
