...
def write_cluster(graph, cluster_community_vc, out_file):

	result_dictionary = {}
	for idx, cluster in enumerate(cluster_community_vc):
		result_dictionary[idx] = graph.vs[cluster]['name']
...
def _main():
	...
	parser.add_argument('--output', help='output folder location', type=str, required=True)
	parser.add_argument('--database', help='database file location for filtering', type=str, required=False)
	parser.add_argument('--names', help='names of proteins id to sub graph', type=list, required=False, default=None)
	parser.add_argument('--alignment_score', help='output file location', type=int, required=False, default=None)
	parser.add_argument('--alignment_weight', help='name of edge attribute', type=str, required=False, default=None)
	parser.add_argument('--alignment_score_rule', help='name of edge attribute', type=str, required=False, default=None)


	options = parser.parse_args()
	if any([item is not None for item in [options.alignment_score, options.alignment_score_rule]]) and not all([item is not None for item in [options.alignment_score, options.alignment_score_rule]]):
		parser.error('the following arguments are required: alignment_score, alignment_weight, alignment_score_rule')
	...
	if options.names:
		print("names = {}".format(options.names))
		db_cluster_vertices = graph.vs.select(name_in = options.names)
		if len(db_cluster_vertices) == 0:
			print("names {}, was not found in graph".format(options.names))
			return
			
		graph.subgraph(db_cluster_vertices)
	
	if all([item is not None for item in [options.alignment_score, options.alignment_score_rule]]):
		...
		graph = graph.subgraph_edges(es_selection(graph, options.alignment_weight, options.alignment_score))
		
	
	cluster_community_dendogram = graph.community_fastgreedy(weights=options.alignment_weight)
	cluster_community_vc = cluster_community_dendogram.as_clustering()

	result_dictionary = {}
	
	result_dictionary['xgmml_file'] = options.xgmml_file
	result_dictionary['type'] = 'greedy'
	result_dictionary['clusters'] = {}
	
	if options.database:
		result_dictionary['database'] = options.database
	
	if options.names:
		result_dictionary['names'] = options.names
	
	if all([item is not None for item in [options.alignment_score, options.alignment_score_rule]]):
		result_dictionary['alignment_score'] = options.alignment_score
		result_dictionary['alignment_weight'] = options.alignment_weight
		result_dictionary['alignment_score_rule'] = options.alignment_score_rule
		
	for idx, cluster in enumerate(cluster_community_vc):
		result_dictionary['clusters'][idx] = graph.vs[cluster]['name']
