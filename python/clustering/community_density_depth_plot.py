...
def _main():
	...
	with open(options.data_file) as json_file:
		...
		algorithms_densities = data
	
	
	clustering_dictionary_queue.append((algorithms_densities[0], []))


	while(len(clustering_dictionary_queue) > 0):
		me, path = clustering_dictionary_queue.pop()
		if len(me['childs']) == 0:
			branches.append(path + [(me['cluster_id'], me['density'], me['n_vertices'], me['n_edges'])])
		else:
			for child in me['childs']:
				clustering_dictionary_queue.append((child, path + [(me['cluster_id'], me['density'],  me['n_vertices'], me['n_edges'])]))
	...
	for index, branch in enumerate(branches):
		...
		n_vertices = []
		n_edges = []
		for cluster_id, density, vertices, edges in branch:
		...
		ax.plot(depths, densities, label='_'.join(map(str,cluster_ids)), color=colors[index], alpha=0.5)
		
		for i, txt in enumerate(n_vertices):
			ax.annotate(txt, (depths[i], densities[i]), alpha=0.5)
