...
def get_edges_le(graph, alignment_score):
	return graph.es.select(edge_weight_tmp_le = alignment_score)

def get_edges_ge(graph, alignment_score):
	return graph.es.select(edge_weight_tmp_ge = alignment_score)
...
def _main():
	...
	if options.reverse:
		...
		alignment_step = -1
		es_selection = edges_function('LE')
		
	else:
		alignment_start = min_alignment_score
		alignment_end = max_alignment_score + 1
		alignment_step = 1
		es_selection = edges_function('GE')

	
	graph.es['edge_weight_tmp'] = graph.es[options.weight]
	...
	for alignment_score in range(alignment_start, alignment_end, alignment_step):
		graph = graph.subgraph_edges(es_selection(graph, alignment_score))
		cluster_community_dendogram = graph.community_fastgreedy(weights=options.weight)
		cluster_community_vc = cluster_community_dendogram.as_clustering()
		modularity = cluster_community_vc.modularity
		modularities[alignment_score] = modularity
		n_vertices[alignment_score] = graph.vcount()
		n_edges[alignment_score] = graph.ecount()
		n_clusters[alignment_score] = len(cluster_community_vc)


	result_dictionary = {}
	
	for i in range(alignment_start, alignment_end, alignment_step):
		result_dictionary[i] = { 'modularity': modularities[i], 'n_vertices': n_vertices[i], 'n_edges': n_edges[i], 'n_clusters': n_clusters[i] }
