...
def _main():
	...
	parser.add_argument('--image', help='output file location', type=str, required=False)
	...
	potential_connections = graph.vcount() * (graph.vcount() - 1) / 2
	alignment_score_counter = Counter(graph.es[options.weight])
	
	min_alignment_score = int(min(alignment_score_counter.keys()))
	max_alignment_score = int(max(alignment_score_counter.keys()))
	alignment_score = min_alignment_score

	while alignment_score <= max_alignment_score:
		density = sum(alignment_score_counter.values()) / potential_connections
		if alignment_score in alignment_score_counter.keys():
			del alignment_score_counter[alignment_score]
		densities.append(density)
		alignment_score+=1


	result_dictionary = {}
	for i in range(min_alignment_score, max_alignment_score + 1):
		result_dictionary[i] = densities[i - min_alignment_score]
	...
	if options.image:	
		...
		ax.plot(list(range(min_alignment_score, max_alignment_score + 1)), densities, '.-')
		ax.set_title('Density over different Alignment Score threshold')
