...
def _main():
	...
	parser.add_argument('--clustering_coefficient_xlsx', help='number of taxonomy to calculate', type=str, required=True)
	...
	print("clustering_coefficient_xlsx = {}".format(options.clustering_coefficient_xlsx))
	...
	taxonomy_links = pd.read_excel(options.clustering_coefficient_xlsx, index_col=0).astype(float)
	...
	ax.set_title('Cluster Coefficient Between Toxonomies - {}'.format(options.taxonomy_level))
