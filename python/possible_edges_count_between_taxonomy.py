...
def get_possible_edges_count_between_taxonomy(graph, find, t_level, min_nodes = 0):
		...
		for t_index in range(s_index, len(taxonomies)):
			...
			if s_taxonomy in find and t_taxonomy in find and taxomony_in_level_counter[s_taxonomy] >= min_nodes and taxomony_in_level_counter[t_taxonomy] >= min_nodes:
				...
				if t_taxonomy not in results[s_taxonomy]:
					if s_taxonomy == t_taxonomy:
						results[s_taxonomy][t_taxonomy] = (taxomony_in_level_counter[s_taxonomy] * (taxomony_in_level_counter[t_taxonomy] - 1)) / 2
					else:
						results[s_taxonomy][t_taxonomy] = taxomony_in_level_counter[s_taxonomy] * taxomony_in_level_counter[t_taxonomy]
...
def _main():
	...
	graph_taxonomy_links = get_possible_edges_count_between_taxonomy(graph, df_relevancy.index, options.taxonomy_level, options.min_size_threshold)
