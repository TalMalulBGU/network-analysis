...
def get_edges_count_between_taxonomy(graph, find, t_level, min_nodes = 0):
   
	results = dict()
	
	taxomony_in_level_counter = Counter(taxonomy for lst_taxonomy in graph.vs[t_level] for taxonomy in lst_taxonomy)
	for e in graph.es:
		for s_taxonomy in graph.vs[e.source][t_level]:
			if s_taxonomy in find and taxomony_in_level_counter[s_taxonomy] >= min_nodes:
				for t_taxonomy in graph.vs[e.target][t_level]:
					if t_taxonomy in find and taxomony_in_level_counter[t_taxonomy] >= min_nodes:
						if s_taxonomy not in results:
							results[s_taxonomy] = dict()
						if t_taxonomy not in results[s_taxonomy]:
							results[s_taxonomy][t_taxonomy] = 0
						results[s_taxonomy][t_taxonomy] += 1
...
def _main():
	...
	graph_taxonomy_links = get_edges_count_between_taxonomy(graph, df_relevancy.index, options.taxonomy_level, options.min_size_threshold)
