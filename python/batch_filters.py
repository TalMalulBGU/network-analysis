...
from tutils.igraph_protein_analysis import *
from tutils.filters.filter_factory import *
...
def _main():
	...
	parser.add_argument('--settings_file', help='the taxonomy relevancy group', type=str, required=True)
	...
	with open(options.settings_file) as json_file:
		...
		filters_order, filters = parser.parse_filters()
		relevancies = parser.parse_relevancies()
		analyses = parser.parse_analyses() 
	...
	df_filters = pd.DataFrame(data={n: [f] for n, f in filters.items()}, columns=filters_order)
	for filter_name in filters_order:
		df = df.explode(filter_name)
