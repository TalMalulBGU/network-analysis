...
def _main():
	...
	while degree <= max_degree:
		ig_vs = graph.vs.select(_degree_ge = degree)
		ig_es = graph.es.select(_incident = ig_vs)
		density = len(ig_es) / potential_connections
	...
	for i in range(len(densities)):
		result_dictionary[min_degree + i] = densities[i]
